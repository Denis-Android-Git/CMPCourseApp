...
final class AES {
    ...
    init(keySize: Int = kCCKeySizeAES256) {
        ...
        let status = SecRandomCopyBytes(kSecRandomDefault, keySize, &keyBytes)

        guard status == errSecSuccess else {
            fatalError("Failed to generate secure random key. Status: \(status)")
        }
    ...
    func decrypt(_ base64: String) -> String {
        guard let fullData = Data(base64Encoded: base64),
              fullData.count > blockSize else { return "" }
